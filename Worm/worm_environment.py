...
class WormSimulationEnv(gym.Env):
    def __init__(self):
        ...
        self.action_space = spaces.Discrete(2)  # 0: No turn, 1: Turn left, 2: Turn right
    ...
    def step(self, action):
        self.worm.update(action, self.food)
    ...
    def _calculate_reward(self):
        ...
        # Reward for not touching the wall remains the same
        
        if 0 < self.worm.position[0] < self.dimx and 0 < self.worm.position[1] < 1200:
            wall_reward = 0.1
        elif not (0 < self.worm.position[0] < self.dimx) and not (0 < self.worm.position[1] < 1200):
            wall_reward = -5
        else:
            wall_reward = -0.1
        ...
        return wall_reward + penalty +wall_reward
