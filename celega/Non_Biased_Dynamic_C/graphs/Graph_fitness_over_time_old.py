...
class Genetic_Dyn_Algorithm:
    ...
    def count_changes(self, candidate_weights, atol=1e-6):
        """Element‑wise count of weights differing from original genome by > atol."""
    ...
    def run(self, env, batch_size: int = 10, jitter_strength: float = 0.0):
        ...
        ax3.set_xlabel("Time (minutes)", fontsize=18)  # timeline axis
        ...
        for filename in os.listdir(full_folder):
            ...
            for batch in batches:
                # fitness – expensive rollout computed in parallel via Ray
                fitness.extend(
                    ray.get(
                        [
                ...
                # distance / change metrics – cheap post‑processing
                dist.extend(
                    [
        ...
        excluded_colours = {"gold", "teal"}     # ⬅️  NEW
        # ── draw curves ──
        for metric in ("fitness", "distance", "changes"):
            for colour, runs in metrics[metric].items():
                if colour in excluded_colours or not runs:   # ⬅️  skip unwanted series
                    continue
        ...
        # Remove the trailing " (mean)" when deduplicating / ordering
        m = {lab.replace(" (mean)", ""): (handle, lab) for handle, lab in zip(h, l)}
        ax1.legend(
            [m[o][0] for o in legend_order if o in m],
