def transform_text(input_text):
    ...
    for line in lines:
        ...
        # Check if the line starts with 'def' to identify function definitions
        if line.startswith('def '):
            # Extract the function name
            func_name = line.split('(')[0].strip().replace('def ', '')

            # Initialize an empty dictionary for the function
            transformed_data[func_name] = {}

        # Check if the line contains 'postSynaptic[' to identify assignment lines
        if 'postSynaptic[' in line:
            # Extract the relevant parts
            parts = line.split('[nextState] += ')
            if len(parts) != 2:
                continue
            
            # Extract the synaptic connection and increment value
            connection = parts[0].split("'")[1]
            increment = int(parts[1].strip())
            
            # Update the transformed data
            if func_name in transformed_data:
                if connection in transformed_data[func_name]:
                    transformed_data[func_name][connection] += increment
                else:
                    transformed_data[func_name][connection] = increment

    # Generate the output format
    output_lines = []
    for func_name, connections in transformed_data.items():
        output_lines.append(f"    '{func_name}': {{")
        for connection, increment in connections.items():
            output_lines.append(f"        '{connection}': {increment},")
        output_lines[-1] = output_lines[-1][:-1]  # Remove the comma from the last entry
        output_lines.append(f"    }},")

    # Join the output lines into a single string with proper indentation
    output_text = '\n'.join(output_lines)
